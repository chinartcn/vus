...
int g_uses_gui = 0; /* 是否在代码中使用了 GuiLite 图形内建函数（决定链接参数） */
int g_uses_png = 0; /* 是否使用 图形_背景图（决定追加 -lpng -lz 链接库） */
int g_uses_freetype = 0; /* 是否使用 图形_字体_加载（决定追加 -lfreetype 链接库） */
...
GenBuf *g_vua_premain = NULL;     /* 收集 界面_绑定 的静态包装函数 */
GenBuf *g_vua_fwd = NULL;         /* 收集包装函数的前向声明（放 include 后） */
int     g_vua_bind_count = 0;     /* 包装函数序号，用于唯一命名 */
int     g_uses_vua = 0;           /* 用户代码是否用了 界面_*（决定 include vua.h）*/
...
static char *gen_expr_call(GenBuf *buf, VusAstCall *call) {
    ...
    /* ============= 1. 基础 IO + 日志 ============= */
    result = gen_builtin_io(buf, call);
    if (result) return result;

    /* ============= 2. VUA 界面内建（Android 组件流，rt/vua.c） =============
     * 多屏导航 + 事件绑定。驱动全局 VuaSession（vua_global_session）。
     * 界面_显示(路径)  读 .vua 压栈成为当前屏；
     * 界面_返回() / 界面_返回至(名)  弹栈导航；
     * 界面_绑定(事件名, 处理函数)  把 .vus 函数登记为当前屏的事件 handler。
     * 注：VUA handler 与 GenBuf 内部 helper 紧密耦合，保留在 generator.c 中。 */
    ...
    /* ============= 3. 图形/GUI + 传感器 + 音频 + TUI ============= */
    result = gen_builtin_gui(buf, call);
    if (result) return result;

    /* ============= 4. 网络 + 文件 ============= */
    result = gen_builtin_net_file(buf, call);
    if (result) return result;

    /* ============= 5. 插件 + JSON ============= */
    result = gen_builtin_plugin(buf, call);
    if (result) return result;

    /* ============= 6. 日期 + Termux ============= */
    result = gen_builtin_date(buf, call);
    if (result) return result;

    /* ============= 7. 数据结构（文本/列表/字典） ============= */
    result = gen_builtin_data(buf, call);
    if (result) return result;
...
static void gen_main_function(GenBuf *buf, VusAstProgram *program, int debug) {
    gen_emit_line(buf, "int main(int argc, char** argv) {");
    buf->indent++;
    ...
    gen_emit_line(buf, "vus_cli_init(argc, argv);");
    gen_emit_line(buf, "int _err = 0;");
    gen_emit_line(buf, "VusError* _vus_err = NULL;");
...
char *vus_generate_c(VusAstProgram *program, VusConfig *config) {
    ...
    if (g_uses_vua) {
        ...
        if (!assembled) {
            ...
            /* 在 int main(int argc, char** argv) { 前插入包装函数 */
            const char *mk = "int main(int argc, char** argv) {";
