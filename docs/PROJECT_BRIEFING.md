...
### 关键需注意（已知限制/未完成）
- 异步 `等待` 已实现：`等待(协程句柄)` 驱动协程到完成并取回其返回值（`VUS_AST_AWAIT` → `vus_coro_await_handle`，可越过多次 `让出`）；`等待(毫秒)` 睡眠语义请用 `睡眠`。
- 函数内部局部变量仅支持函数顶收集式声明；布尔相关 `返回` 值使用注意。
- 网络函数优先 `VUS_HAVE_CURL`（`-lcurl`），缺失时自动回退系统 `curl` 命令；进程内 Python 需 `VUS_USE_PY`（缺失/加载失败时 `.vux` 插件回退子进程，`插件_运行JSON` 亦可用）。
