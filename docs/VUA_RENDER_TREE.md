...
真抽屉控件（滑出/收起侧栏）为已登记内置 `type`，渲染树节点（`菜单`/`内容`/`宽度`/`侧` 原样透传，`事件` 归一为 `event`）：
...
```json
{
  ...
  "菜单": [ "概览", { "标题": "文档" } ],
  "内容": [ { "type": "文本", "内容": "主区" } ],
  "event": { "name": "切页", "collect": [] },
  "宽度": 240,
  "侧": "左"
}
```

Java 侧（`VuaRenderer.drawerView`）：`内容` 递归建主内容区，`菜单` 数组逐项建行；菜单项点击触发 `event` 所指向事件，载荷与 `列表` 行点击一致——`{"下标":<菜单索引>,"值":"<菜单标题>"}`（再合并 `collect` 收集变量与 `callback` 回调变量）；`宽度` 为面板宽度 dp，`侧` 为 `左`/`右`（缺省左）。默认收起，`侧` 对应边缘 20dp 水平拖出、点遮罩或菜单项收起。
