...
> **约定**：本文**不涉及 GUI**（`图形_*`、guilite、guilibridge、fbprobe、x11_read 等）——该部分尚在开发中，不在本文覆盖范围内。
...
| `睡眠(毫秒)` | 休眠指定毫秒 | 生成 `vus_thread_sleep(...)` |
...
> **⚠️ 已知问题（如实标注）**：`睡眠(毫秒)` 对应生成 `vus_thread_sleep(毫秒)`，但在 `rt/libvus_rt.c` 与 `rt/vus_coro.c` 中**均未定义 `vus_thread_sleep` 符号**（源码中只有引用、无定义）。因此**当前 `睡眠()` 会导致链接失败**，属于「关键字/语法已接好、运行时符号缺失」的未完成项。`让出`/`恢复`/`线程`/`等待线程` 的运行时函数存在且可用。
...
| `睡眠(毫秒)` | `vus_thread_sleep` | ⚠️ 运行时符号缺失，**当前不可链接**（见 10.3）|
...
### 11.7 插件 / JSON 内置函数（依赖 `VUS_USE_PY`）

| 调用 | 运行时 | 说明 |
|------|--------|------|
| `插件_运行(插件, 命令)` | `vus_plugin_run_vux` | 在子进程中运行 `.vux` Python 插件 |
| `插件_运行JSON(插件, 命令)` | `vus_plugin_run_vux_inproc` / `vus_plugin_run_vux_json` | 嵌入解释器调用插件；返回结构化结果 |
| `JSON_解析(字符串)` | `vus_json_parse` | JSON → `VusObject*`（列表/字典/字符串）|
| `JSON_生成(对象)` | `vus_json_generate` | `VusObject*` → JSON 字符串 |

> **依赖提示**：`vus_plugin_run_vux_json`、`vus_json_parse`、`vus_json_generate`、`vus_typeof` 若未以 `VUS_USE_PY` 编译（`#else` 降级分支），JSON 解析/生成返回空、`typeof` 恒返回 `"空"`。即**默认构建下 JSON 结构化函数不可用**，需开启 `VUS_USE_PY`。
...
> 结论：**当前真正可用的内置函数** = 11.1 的核心 + 11.2 的日志 + 11.3–11.6 的四组插件运行时函数 + 11.7 的插件/JSON（受限编译配置）。`图形_*`（GUI）不在本文范围。
...
| 线程 | `线程`/`等待线程` | ✅ 已实现 |
| 协程 | `协程`/`恢复`/`让出` | ✅ 已实现 |
| 睡眠 | `睡眠(毫秒)` | ❌ 生成 `vus_thread_sleep`，**运行时无符号定义→链接失败** |
...
| 插件/JSON | `插件_运行`、`插件_运行JSON`、`JSON_解析`/`生成` | ⚠️ 仅 `VUS_USE_PY` 编译可用；默认降级（JSON 返回空 / typeof 恒 `"空"`）|
| 旧式标准库 | 断言/退出/长度/拼接/分割/替换/取子串/取整/遍历列表/字典操作/文件旧名等 | ❌ **未实现**（generator 无接线）|
| 手动内存管理 | 手动申请/释放 | ❌ 不存在（仅自动引用计数）|
| GUI | `图形_*`/guilite/guilibridge/fbprobe/x11_read | 不在本文范围（开发中，未完成）|