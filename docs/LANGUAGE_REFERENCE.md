...
| `等待` | `VUS_TOKEN_CN_AWAIT` | `等待(协程句柄)` 真 await：驱动协程到完成并返回其返回值 |

> `等待`（AWAIT）在 v3.0 后续版本已实现：parser 将其消费为 `VUS_AST_AWAIT`，生成 `vus_coro_await_handle(句柄)`。等待时不断 `vus_coro_resume` 直到协程 `done`（可越过协程内部多次 `让出()`），随后读取协程入口经 `vus_coro_store_result` 回填的结果槽并返回。用法示例：

```vus
c = 协程(任务, 实参)   # 创建协程
结果 = 等待(c)          # 真异步等待：驱动到完成并取回返回值
```

> **函数一等公民**（v3.0 后续）：`函数值(函数名)` 把具名函数装箱为 `VusObject(TYPE_FUNC)`，可存入变量/列表/字典作为值传递；`调用(函数值, 实参...)`（别名 `调用函数`）解包函数值并调用，返回其返回值。运行时 `VusObject` 增加 `TYPE_FUNC` 类型与 `vus_object_func`/`vus_object_func_call`。

> **多返回值**（v3.0 后续）：`返回 值1, 值2, ...` 将多值打包为列表（`VUS_AST_RETURN_MULTI`）放入返回值槽；调用侧 `a, b, c = 函数()`（`VUS_AST_MULTI_ASSIGN`）从列表逐项拆包赋值，兼容函数内局部变量与顶层全局变量（自动登记全局声明）。

> **f-string**（v3.0 后续）：`f"文本{表达式}文本"` 由词法器产生 `VUS_TOKEN_FSTRING`，解析器拆分为文本段与插值段后串成 `..` 拼接链；插值表达式经独立子词法器/解析器递归求值；`{{` `}}` 折叠为字面花括号，`\{` 转义字面花括号。

> **复合赋值**（v3.0 后续）：`a += b`、`a -= b`、`a *= b`、`a /= b`、`a %= b`、`a ..= b` 由词法器识别 `VUS_TOKEN_*_ASSIGN`，解析器展开为 `a = a <op> b`（`..=` 对应字符串拼接）。
...
| 逻辑 | `非`/`not`、`!`（等价 `非`/`not`）| 逻辑非（一元）| 按字符串是否等于 `"true"` 取反 |
| 位运算 | `&` `\|` `^` | 与 / 或 / 异或 | 整数位运算 |
| 位运算 | `<<` `>>` | 左移 / 右移 | 整数移位 |
| 位运算 | `~` | 按位取反（一元）| 整数取反 |
...
> - `+ - * / % & | ^ << >> .. == != < > <= >= and or not 非 和 或 ~ !` 均已实现（`!` 由 lexer 归为 `NOT`，与 `not`/`非` 完全同一路径）。
> - AST 中 `VUS_AST_BINARY_OP`/`VUS_AST_UNARY_OP` 一律生成上述代码。
> - 一元分支中解析器接受 `-`、`not`/`非`/`!`（lexer `!`→`NOT`）、`~`（位补充运算符未在解析器分支出现，属「未实现」）。
...
- **字典**：`{键: 值, …}` 生成 `VusObject`（`TYPE_DICT`）。**字典遍历接口已提供**：
  - `循环 键 在 字典` —— 直接遍历字典的**键**（等价于先 `字典_键` 再遍历；生成 `vus_dict_keys_of` + 列表遍历）；
  - `字典_键(字典)` —— 返回所有键的列表；
  - `字典_项(字典)` —— 返回所有键值对列表，每项为 `[键, 值]` 双元素列表，配 `循环 对 在 字典_项(字典)` 可**同时拿键与值**（用 `对[0]`/`对[1]` 下标读取）。
  键值读写用 `字典_设值(字典, 键, 值)` / `字典_取值(字典, 键)`。（`vus_object_to_string` 对字典返回 `"{}"` 占位属旧的序列化占位说明）。
- **空**：`null`/`空` 生成 `NULL`。
...
- **字典遍历**：迭代容器为字典（`VusObject* TYPE_DICT`）时走 `vus_dict_keys_of` + 列表遍历（见 6.2）；`字典_项` 返回 `[键,值]` 对列表，供第⑤种形式同时取键值。
...
```vus
尝试:
    抛出 "错误1"          # throw 一条消息（字符串），类型=默认"错误"
    抛出 值错误, "传参非法"  # throw (类型, 消息)
    抛出 网络错误           # throw 单标识符=类型名，消息为空
    打印("这行不会执行")
捕获 值错误:
    打印("按类型捕获")
捕获 网络错误, 文件错误:   # 多类型（取第一个为代表 + 均可用 vus_error_matches）
    打印("按类型捕获（多类型）")
捕获:
    打印("通配捕获")
```
- `抛出`/`throw`（`VUS_AST_THROW`）支持三种形态，生成 `_vus_err = vus_error_new_typed(1, <类型>, <消息>, __LINE__, __func__)`，并以 break 跳出当前 try 的 `do{…}while(0)`：
  - `抛出 消息`：消息可以是任意表达式，异常类型记为默认 `"错误"`（旧写法）；`抛出 拼接(…)` 等动态消息依然可用；
  - `抛出 类型, 消息`：第一个表达式为**异常类型**（裸标识符按类型名字符串处理；其它表达式求值取串），第二个为消息；
  - `抛出 类型名`：单裸标识符视为异常类型（消息为空），如 `抛出 网络错误`。
- `尝试`/`try`（`VUS_AST_TRY`）：生成保存/恢复错误态的块：
  1. 保存 `_saved_err`，清空 `_vus_err`；
  2. try 体放入 `do{…}while(0)`；
  3. 依序匹配 `捕获` 子句（链式 `if/else if/else`）：
     - **带类型**的捕获：`vus_error_matches(_vus_err, "类型名")`——**类型名与 `err->type` 相等，或与 `err->msg` 相等**均命中（后者兼容旧「消息即类型」用法）；
     - **通配**捕获（无类型）：`else { ... }`；
...
- 未捕获异常在最外层处理并终止程序。
- **内建异常类型系统**：`VusError` 带 `type` 字段（默认 `"错误"`），`抛出 类型, 消息` 指定类型；类型名为约定字符串（如 `值错误`/`类型错误`/`索引错误`/`键错误`/`文件错误`/`网络错误`/`超时错误`），由用户脚本约定使用，不做硬性枚举。错误打印为 `E001[类型] 消息`。
...
> 桌面未定义 `VUS_HAVE_CURL` 时自动回退系统 `curl` 命令（需 PATH 中有 curl）；curl 也不可用才返回错误消息。
...
| `字典_键(字典)` | `vus_dict_keys_of` | 返回字典键列表 |
| `字典_项(字典)` | `vus_dict_items` | 返回字典键值对列表：每项为 `[键, 值]` 双元素列表，配 `循环 对 在 字典_项(字典)` 遍历键值 |
...
### 11.10 已废弃 / 保留的内置函数（务必区分）

设计文档 §10.1 中的**旧式标准库名称已于 v3.0 正式版后接线**到现代实现（`generator.c` 的 `gen_expr_call` 已映射）：`长度`、`拼接`、`分割`、`替换`、`取子串`、`取整`、`取随机数`、`创建列表`、`添加元素`、`取元素`、`删除元素`、`列表长度`、`遍历列表`、`创建字典`、`字典设值`、`字典取值`、`字典删除`、`字典长度`、`读取文件`、`写入文件`、`追加文件`、`删除文件`、`文件是否存在`、`当前时间`、`调试输出`、`退出`、`断言` 均可直接调用，语义与上文的 `文本_*`/`列表_*`/`字典_*`/`文件_*`/`日期_*` 版本一致（`长度` 对字符串返回 UTF-8 字节长度、对列表/字典返回元素个数；`断言` 仅条件为 `"true"` 才通过，失败打印消息并以退出码 1 终止，不触发异常链）。

> 说明：`等待(毫秒)` 睡眠 **不接线**——`等待` 是协程保留词（`VUS_TOKEN_CN_AWAIT`，等待协程完成取回结果），旧文档中的「等待 = 睡眠」语义请使用 `睡眠`。`调试输出` 仅在 `vus run --debug` 时输出。

> 结论：**当前真正可用的内置函数** = 11.1 的核心 + 11.2 的日志 + 11.3–11.6 的四组插件运行时函数 + 11.7 的插件/JSON + **11.8–11.9 的 GUI**（`图形_*` 画布流、`界面_*` 组件流）+ 上表旧式标准库名称。
...
- **`断言`**：已实现（旧式标准库接线）。仅条件为 `"true"` 才通过；失败打印消息并 `exit(1)` 终止，不经过异常链（见 §11.10）。
...
throw_stmt     := ("throw"|"抛出") (expr "," expr    // 类型, 消息
                                  | IDENT           // 单标识符 = 类型名（消息空）
                                  | expr)           // 单表达式 = 消息（类型=默认"错误"）
...
| 协程 | `协程`/`恢复`/`让出` | ✅ 已实现 |
| 睡眠 | `睡眠(毫秒)` | ✅ 已实现 |
| 异步 `等待` | `等待(协程句柄)` | ✅ 已实现：驱动协程到完成并返回其返回值 |
...
| 插件/JSON | `插件_运行`、`插件_运行JSON`、`JSON_解析`/`生成`/`查询`、`对象文本`、`字典_键`、`字典_项` | ✅ 已实现；JSON 走内嵌 yyjson（默认可用）；`typeof` 与嵌入式插件调用需 `VUS_USE_PY`（否则 typeof 恒 `"空"`、进程内调用回落子进程）|
| 旧式标准库 | 断言/退出/长度/拼接/分割/替换/取子串/取整/取随机数/创建列表/添加元素/取元素/删除元素/列表长度/遍历列表/创建字典/字典设值/字典取值/字典删除/字典长度/读取文件/写入文件/追加文件/删除文件/文件是否存在/当前时间/调试输出 | ✅**已接线**（§11.10；语义同 `文本_*`/`列表_*`/`字典_*`/`文件_*`/`日期_*`）|
