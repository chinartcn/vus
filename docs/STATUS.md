...
# VUS 编译器 v3.0.20260904150204（正式版） — 状态报告

> 本文反映 `/workspace/vus` 当前真实代码状态，权威依据为 `docs/LANGUAGE_REFERENCE.md`、`docs/ARCHITECTURE.md` 与 `docs/API_REFERENCE.md`。
> 说明：早期文档（X11 探针时代）约定「不涉及 GUI」；现已落地 **GuiLite 画布流**（`图形_*`）与 **VUA Android 组件流**（`.vua` + `界面_*`），本报告一并列入已实现功能。
...
- 一元运算符支持：`-`、`非`/`not`、`!`（lexer 归为 `NOT`，与 `非` 等价）、`~`
...
- 内建函数 + 日志 + TUI/网络/文件/日期 + 文本/列表/字典 + JSON + 音频 + GUI(`图形_*`/`界面_*`) 等插件运行时函数映射
- **生成代码优化**（近期专项）：函数体特征扫描按需生成 `_vus_result`/`_vus_err`、`vus_var_set` 赋值热路径、列表/字典字面量 helper（`vus_object_list`/`vus_object_dict`）、整数字面量池化（`vus_literal("7")`）、转文本直接转发入参、循环模板收敛（单行块防缩进爆炸）
...
- 字典 `VusDict`（链地址法哈希表，DJB2 风格，自动扩容；支持 `字典_键`/`字典_值`/`字典_项`/foreach 遍历）
- `VusObject` 结构化容器（魔数 `VUS_OBJECT_MAGIC`，承载列表/字典/字符串/函数值 `TYPE_FUNC`）
- 闭包/函数一等公民：`VusClosure`（运行时回调）+ `函数值()` 装箱 + `调用()` 解包（语言层新增，见「测试中/待开发」表）
- 错误处理 `VusError`（错误码链，Go 风格，非 setjmp）
- 线程（pthread）与协程（`rt/vus_coro.c`，setjmp/longjmp + 汇编换栈，128KB 独立栈）句柄注册表各 64 槽；真 await：`vus_coro_await_handle` 驱动协程到 done 并返回 `vus_coro_store_result` 回填的结果
- 栈追踪（`VUS_MAX_STACK_DEPTH=256`）、分级日志（EasyLogger 惰性初始化）
- 插件运行时函数：TUI/网络/文件/日期/插件调用/JSON/文本/列表/字典/音频/GUI
- **字符串拼接免二重复制**（`vus_string_concat` 直接写目标串）、**`VusString` 头+负载单块 malloc**、**`vus_to_string` 整数值驻留缓存**（热循环提速）、**`vus_literal` 字符串常量池**、**`vus_var_set` 统一赋值引用计数**
- GUI 运行时：GuiLite 桥接（`guilite_bridge.c` + C++ 包装 + X11/headless 平台层 + GLES 可选）、yyjson、gifdec、nanosvg、FreeType；VUA（`vua.c`，Android 组件流会话/屏栈/渲染树）
...
- `vus chart <音频> [-o 文件]` — 生成体感音游谱面 JSON
- `vus lsp` — 语言服务器（JSON-RPC 补全，可集成 ACode/GUI Designer）
...
- 已知缺陷已修复（2026-09）：vus.json 解析死循环、source 过早释放致导出空函数名、插件 .o 重复 `main` 冲突（新增 `omit_main` 库式编译）

### GUI 与渲染 — 已实现

**画布交互流（`图形_*`，GuiLite）**：
- 绘制原语：`图形_初始化` `图形_画点/画线/矩形/填充/画圆/填充圆/圆弧/圆环/圆角矩形/圆角填充` + `图形_刷新` `图形_保持`
- 控件与状态：按钮、开关、单选、复选框、文本框、微调、滑块、进度、面板、卡片、表格行、列表（含行点击/选中）、滚动容器、画布（点/命中）、动画（打开/帧数/下一步/关闭）、页面（打开/当前/返回/绘制）、标签、文字、MD 多行文本、图片（PNG/SVG/GIF）、背景图、字体（加载/已加载）、主题、外观
- 事件：`图形_取事件`、按钮点击、行点击、滑块值、开关值、单选值、微调值、滚动容器偏移/滚、悬停、鼠标位置/按键/按键码/滚轮、键按下、模拟点击
- 依赖：GuiLite（rt/guilite）+ X11 平台层（rt/guilite_platform.c，可选 GLES）+ libpng/freetype/nanosvg/gifdec；由生成器自动探测并追加链接参数
...
**终端 UI（TUI，基于 ANSI 转义码，无外部依赖；流式 + 画布帧缓冲差分刷新）**：
| 函数 | 说明 |
|------|------|
| `tui_清屏()` | 清空终端屏幕 |
| `tui_重置()` | 重置终端属性 |
| `tui_设置颜色(前景色, 背景色)` | 设置 ANSI 256 色（0~255，越界钳制） |
| `tui_定位(行, 列)` | 移动光标 |
| `tui_进度条(当前值, 总值, 宽度)` | 显示进度条 |
| `tui_画布(行数, 列数)` | 创建/调整帧缓冲画布 |
| `tui_绘制文本(行, 列, 文本[, 前景色, 背景色, 样式])` | 文本写入画布（样式位：1=加粗 2=下划线 4=反显） |
| `tui_边框(行, 列, 高, 宽[, 标题])` | 画布绘制 Unicode 边框（标题居中） |
| `tui_刷新()` | 差分上屏：仅输出与上一帧不同的单元格 |
| `tui_清行(行)` | 清空画布/终端指定行 |
| `tui_光标(显隐)` | 显示/隐藏光标 |
| `tui_文本样式(样式)` | 流式文本样式（0 复位） |
| `tui_终端尺寸()` | 查询终端尺寸 `"行,列"`（无终端回退 `"24,80"`） |
| `tui_读取按键()` | 非阻塞读键（含 ESC 组合序列；无终端返回空串） |
| `tui_输入文本(提示[, 最大长度])` | 行编辑输入（回显/退格/回车；无终端返回空串） |
...
| — | **桌面未定义 `VUS_HAVE_CURL` 时自动回退系统 `curl` 命令（需 PATH 中有 curl）** |

**网页服务（rt/vus_web.c；极简 POSIX HTTP 服务器，零外部依赖，后台线程常驻；三引号多行字符串 `"""..."""` 支持跨行与 `{表达式}` 插值）**：
| 函数 | 说明 |
|------|------|
| `网页_服务(端口, 内容)` | 内容模式：任意路径返回该 HTML（配合三引号模板直接出页） |
| `网页_服务目录(目录[, 端口])` | 目录模式：静态文件服务；目录下 `网页.json` 可配 `{"入口":"index.html","端口":8080}`（入口自动、端口参数覆盖配置，缺省 8080） |
| `网页_停止(端口)` | 停止该端口服务（`"1"` 成功 / `"0"` 未找到） |
| 说明 | 三引号字符串：`"""文本{变量}文本"""`，保留换行、支持插值与 `\{` 转义字面花括号，HTML 模板无 4KB 截断；目录模式含 MIME 表、404、URL 解码与 `../` 穿越防护（403） |

**AI 聚合模块（rt/vus_ai.c；统一封装国内常用服务商的 OpenAI 兼容对话接口，桌面回退 curl，无额外依赖）**：
| 函数 | 说明 |
|------|------|
| `AI_api池([配置JSON])` | 导入 `{服务名: 令牌}` 池（支持 `API_` 前缀自动剥除）；无参返回已注册服务 JSON（令牌脱敏） |
| `AI_open兼容(服务名, 地址)` | 为服务绑定 OpenAI 兼容端点（覆盖内置默认：glm/qwen/doubao/deepseek/kimi/siliconflow/spark/hunyuan 等，含中文别名 智谱/千问/豆包/深度求索/月之暗面/硅基流动/讯飞星火/混元） |
| `AI_请求(服务名, 模型[, 消息])` | 发起单轮对话，返回回答文本；消息缺省"你好"；配置缺失返回 `{"错误":"..."}` 可 JSON 解析 |
| `AI_模型列表(服务名)` | 网络请求 `GET {base}/models`，返回该内置服务商可用模型 JSON 数组（需已配置令牌；仅内置服务商支持） |
| `AI_服务列表()` | 返回逗号分隔的已注册服务名 |
| `AI_移除(服务名)` | 从池中移除服务（`"1"` 成功 / `"0"` 未找到） |
| `AI_启动工作流(槽名, 函数, 参数)` | 后台协程执行工作流体（`定义 AI_工作流_xxx:`）；驱动到首个让步点，同名槽启动即刷新状态 |
| `AI_推进(槽名)` | 把工作流推进一步（越过一个让步点），完成后返回 `"0"` |
| `AI_返回(值)` / `AI_完成返回()` | 工作流体内汇报进度 / 宣告完成（均让出，等待 `AI_推进`） |
| `AI_回调(槽名)` | 返回该工作流**最近一次 `AI_返回`** 的值——不管是否结束均可读；结束（`AI_完成返回`）后仍保留最后结果 |
| `AI_清空回调(槽名)` | 把回调值复位为空串（`"1"` 成功；不清空则回调值一直保留） |
| `AI_回调_完成(槽名)` | `"1"`/`"0"`：是否已调用过 `AI_完成返回` |
| `AI_工作流状态(槽名)` | 三态：`"不存在"` / `"进行中"` / `"完成"`（重新启动同名槽后刷新为进行中） |
| `AI_文生图(服务名, 模型, 提示词[, 尺寸])` | 调用 `{base}/images/generations`，返回图片 URL（智谱 glm-image/cogview 等 OpenAI 兼容） |
| `AI_图生图(服务名, 模型, 图片路径, 提示词[, 强度])` | 读本地图片→Base64 内嵌后同端点提交（智谱 glm-image 系）；成功返回图片 URL |
...
| 函数 | 说明 |
|------|------|
| `文本_长度(s)` / `文本_子串(s, 起, 长)` / `文本_分割(s, 分隔)` / `文本_行(s)` / `文本_取字符(s, 索引)` | 字符串工具（`文本_分割/文本_行` 直接返回列表，P7） |
| `列表_追加(列表, 值)` / `列表_取(列表, 索引)` / `列表_长度(列表)` | 列表操作 |
| `字典_设值(字典, 键, 值)` / `字典_取值(字典, 键)` / `字典_键(字典)` | 字典操作 |
...
| `JSON_查询(json, 路径)` | JSON 路径查询 |
| `命令_执行(cmd)` | 执行 shell 命令并捕获输出 |
| — | 部分函数仅在 `VUS_USE_PY` 编译下可用；默认构建 JSON 解析/生成返回空、`类型` 恒返回 `"空"` |

**音频函数（`vus_xyz`/音频驱动）**：
| 函数 | 说明 |
|------|------|
| `音频_打开(路径)` / `音频_播放()` / `音频_暂停()` / `音频_续()` | 音频控制 |
| `音频_跳转(秒)` / `音频_进度()` / `音频_时长()` | 定位与查询 |
...
| test_exception_types.vus | 内建异常类型系统（`抛出 类型, 消息` / `捕获 类型`） | 新增 |
...
| test_vus_lang.vus | 语言插件管理系统 | 新增 |
| test_vus_plugin.vus | 插件系统 | 新增 |
| test_gui_*.vus（11 个） | GuiLite 画布流控件/绘制/页面/图媒体 | ⚠️ 需 X11 显示（无显示环境段错误，非逻辑错误） |
| test_xyz_basic.vus | 体感音游运行时 | 新增 |
| test_comprehensive.vus | 综合功能测试 | ✅ 稳定 |
| test_multi_return.vus | 多返回值（`返回 a,b,c` + `a,b,c = 函数()`） | 新增 |
| test_await.vus | 真 await（等待协程并取结果） | 新增 |
| test_await_multi.vus | await 越过多次 `让出` | 新增 |
| test_function_value.vus | 函数一等公民（`函数值`/`调用`/存列表） | 新增 |
| test_fstring.vus | f-string 插值（含 `{{}}` 折叠、表达式） | 新增 |
| test_composite_assign.vus | 复合赋值 `+= -= *= /= %= ..=` | 新增 |
...
> 说明：`tests/` 下另有 `run_tests.sh`、`run_error_tests.sh` 批量脚本，`vus test` 与 `make test` 驱动。`test_gui_*` 依赖 X11 显示（headless 环境段错误属环境问题）；非 GUI 用例当前全部通过。
...
| 闭包/函数一等公民 | ✅ 已实现 | `函数值(名)` 把函数装箱为 `VusObject(TYPE_FUNC)`，`调用(函数值, 实参...)` 解包调用；函数值可存变量/列表/传递并跨作用域调用，为闭包三特性铺垫 |
| 多返回值 | ✅ 已实现 | `返回 a, b, c` 打包为列表（`VUS_AST_RETURN_MULTI`）；`a, b, c = 函数()` 多目标赋值拆包（`VUS_AST_MULTI_ASSIGN`），兼容函数内局部与顶层全局 |
| f-string | ✅ 已实现 | `f"文本{表达式}文本"` 串接为 `..` 拼接链；`{{`/`}}` 折叠为字面花括号，插值内支持表达式递归求值 |
| 复合赋值 | ✅ 已实现 | `+=` `-=` `*=` `/=` `%=` `..=` 展开为 `a = a op b`（`VUS_TOKEN_*_ASSIGN`） |
| 字典遍历 / 列表下标 | ✅ 已实现 | 遍历：`循环 键 在 字典`/`字典_项(字典)`；下标：`列表[索引]` 用 `vus_list_unwrap` 解包（兼容装箱），`字典_键/取值/设值` 提供字典读写 |
| 异步 `等待`（await） | ✅ 已实现 | `等待(协程句柄)` → `VUS_AST_AWAIT` → `vus_coro_await_handle` 驱动协程到完成（可越过多次 `让出`）并返回协程结果；协程入口 `vus_coro_store_result` 回填结果槽 |
| `!` 逻辑取反 | ✅ 已实现 | 与 `非`/`not` 等价：lexer `!`→`NOT`，parser 单目 op 归 `not` 并接线生成 |
| 手动内存管理 | ❌ 不存在 | 仅自动引用计数，无手动申请/释放 API |
| 类型注解强校验 | ⚠️ 未实现 | 类型注解仅 AST 记录/注释，不强制检查，动态类型 |
| 旧式标准库函数 | ✅ 已接线 | `断言`/`退出`/`长度`/`拼接`/`分割`/`替换`/`取子串`/`取整`/`取随机数`/`创建列表`/`添加元素`/`取元素`/`删除元素`/`列表长度`/`遍历列表`/`创建字典`/`字典设值`/`字典取值`/`字典删除`/`字典长度`/`读取文件`/`写入文件`/`追加文件`/`删除文件`/`文件是否存在`/`当前时间`/`调试输出` 已映射到 `文本_*`/`列表_*`/`字典_*`/`文件_*`/`日期_*` 现代实现；`等待(毫秒)` 仍为协程保留语义（用 `睡眠`）|
...
| # | 描述 | 状态 | 优先级 |
|---|------|:----:|:------:|
| 1 | ~~异步 `等待`（AWAIT）仅为已定义 token，解析器未实现~~ → **已实现**：`等待(协程句柄)` 驱动协程到完成并取回其返回值（`VUS_AST_AWAIT` → `vus_coro_await_handle`，可越过多次 `让出`；`tests/test_await.vus` / `test_await_multi.vus` 通过） | 已实现 | - |
| 2 | 函数内局部变量采用「函数顶部收集声明」（`gen_collect_locals`），非完整词法作用域；对未显式声明的嵌套赋值等场景存在局限 | 已知限制 | 中 |
| 3 | ~~引用计数 `vus_unref` 归零后仅 `free()`，不递归释放容器内部成员，存在内存泄漏~~ → **已修复**：`vus_unref` 归零时按容器类型递归释放内部列表/字典成员（`vus_release_list`/`vus_release_dict`/`vus_object_release`），并带环/深度防护（`rel_chain`，上限 256），避免无限递归与重复释放 | 已修复 | 中 |
| 5 | ~~`抛出的异常类型`目前即「抛出 的消息字符串」，`捕获 <名称>` 为字符串比对，无内建异常类型系统~~ → **已实现**：`VusError` 新增 `type` 字段（默认 `"错误"`），`抛出 类型, 消息` 显式指定类型；`捕获 <类型>` 经 `vus_error_matches` 按类型名（兼容旧「消息即类型」）匹配，多类型/通配捕获、异常冒泡均支持（`tests/test_exception_types.vus` 7 场景通过） | 已实现 | - |
| 6 | ~~结构体构造走通用函数调用模板，与 R1 的「返回槽归还 vus_unref」冲突：构造返回 ref=0 → 被 unref 到 -1 直接 free → 调用方悬垂 UAF（test_struct_chain 段错误；嵌套构造放大堆复用覆写）~~ → **已修复**：`gen_expr_call` 对结构体名提前分发到专用构造模板 `gen_struct_inst_expr`（无 A'/B，与 `VUS_AST_STRUCT_INSTANTIATE` 分支一致），test_struct_basic/chain 通过 | 已修复 | - |
...
- **内存管理**：引用计数自动管理；`vus_unref` 归零时**递归释放**容器内部成员（列表元素/字典键值），并对环引用与嵌套深度做防护（`rel_chain` 256 上限），结构化容器组合可整体回收。
- **表达式临时对象无主计数（R6，待办）**：`x = x + 1`、`转数字(s)` 等「新建」表达式结果残留构造 1 份引用（`vus_var_set` 无差别 +1），长循环中 RSS 线性增长（实测 2×10⁷ 加法 ≈ 780MB；与函数调用边界无关——R1 已让函数路径精确化）。根治需生成器 RAII 式丢弃 +「移动/共享」信息流判断，见 `PERFORMANCE.md` 3.8。
- **`vus_eval` 开销**：每次调用都编译 C 代码并启动子进程，且输出截断为 4096 字节，不适合高频/大数据量求值。
- **字典遍历**：已支持——`循环 键 在 字典`（`vus_dict_keys_of`）、`字典_项(字典)` 返回键值对列表（`vus_dict_items`）；JSON 生成对无遍历的场景已由上述接口补齐。
- **字典/列表序列化**（R4）：`对象文本`/`转文本(对象)` 输出真实内容——列表 `[1, 2, 3]`、字典 `{键: 值, ...}`（递归嵌套支持），取代原字典 `"{}"` 占位；序列化改增长缓冲一次拼装（原逐元素 concat ≈ 2n 次 malloc）。
- **句柄表上限**：线程/协程句柄注册表各 `VUS_MAX_HANDLES=64` 槽，溢出返回 `"-1"` 并回收；长期大量创建线程/协程可能触发上限。
- **网络依赖可选**：`网络_*` 桌面优先 libcurl（`VUS_HAVE_CURL` + `-lcurl`），缺失时自动回退系统 `curl` 命令（需 PATH 中有 curl），能力不降级；APK 走 Java 平台桥。若 curl 也不可用则空串/错误。
- **进程内 Python 依赖编译配置**：`.vux` 插件进程内嵌入需 `VUS_USE_PY`（存在 libpython）编译；未定义或 dlopen 失败时回退子进程 `vux_plugin_manager`（`插件_运行JSON` 回退子进程并按 JSON 解析）。`JSON_解析/生成/查询` 走内置 yyjson，**不依赖** Python；`类型` 在未启用 `VUS_USE_PY` 时恒返回 `"空"`（降级实现）。
...
| 后期演进 | 2026-07~09 | ORM/集成经验脚本、协程 Clang/ARM64 兼容修复、**GuiLite 画布流落地**（`图形_*` 控件/事件/图片/字体）、**VUA Android 组件流落地**（`.vua`+`界面_*`，YJ/RenderTree）、**体感音游**（chart + vus_xyz）、**vaz 扩展包**、**性能优化**（编译器/生成代码/运行时：字符串常量池、单块分配、整数值缓存、helper 收敛、循环模板收敛等，生成代码体积 -45%、编译 -29%、热循环 +47%）、**插件系统三处致命缺陷修复**（vus.json 死循环、vusx 空导出名、重复 main） |
