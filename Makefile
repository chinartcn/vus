...
CORDIS_SRC = $(RT_DIR)/vus_cordis.c
...
vus: $(OBJS) $(YYJSON_OBJ) $(VUA_OBJ) $(RT_OBJ) $(RT_CORO_OBJ) $(RT_C_IMPL_OBJ) $(CORDIS_OBJ) $(EL_OBJ) \
...
# 编译 Cordis 式上下文元框架（服务容器/DI/effect/事件四模式，随运行时库归档）
$(CORDIS_OBJ): $(CORDIS_SRC) $(RT_DIR)/vus_cordis.h $(RT_H) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -I$(RT_DIR) -c -o $@ $<
...
$(RT_LIB): $(RT_OBJ) $(RT_CORO_OBJ) $(RT_C_IMPL_OBJ) $(CORDIS_OBJ) $(YYJSON_OBJ) $(EL_OBJ) $(GUI_OBJ) $(XYZ_OBJ) $(GLES_ARCHIVE_OBJ) \
           $(ONIG_OBJ) $(VUS_EXT_OBJ) $(MINIZ_OBJ)
	ar rcs $@ $^
...
$(RT_SO): $(RT_OBJ) $(RT_CORO_OBJ) $(RT_C_IMPL_OBJ) $(CORDIS_OBJ) $(YYJSON_OBJ) $(EL_OBJ) $(GUI_OBJ) $(XYZ_OBJ) $(GLES_ARCHIVE_OBJ) \
          $(ONIG_OBJ) $(VUS_EXT_OBJ) $(MINIZ_OBJ)
