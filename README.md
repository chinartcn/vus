...
- **插件运行时函数**（已实现）：
  ...
  - **网络**：`网络_GET` `网络_POST` `网络_请求`（自定义头/超时/重试，token 认证） `文件_上传`（multipart） `网络_下载`（桌面优先 libcurl / `VUS_HAVE_CURL`，缺失时自动回退系统 `curl` 命令；APK 走 Java 平台桥无需依赖）
...
> ⚠️ **如实说明（请勿按旧文档误用）**：
> - 异步 `等待` **已实现**：`等待(协程句柄)` 驱动协程到完成并取回其返回值（可越过多次 `让出`）；同步睡眠请用 `睡眠`（`等待(毫秒)` 旧语义不接线）。
> - **类型注解不执行强类型检查**、**泛型目前为语法形态**（类型实参仅作为注释注入，无真实实例化），与设计文档的「强类型」存在差距。
> - 旧式标准库名（`长度`/`拼接`/`分割`/`替换`/`取子串`/`取整`/`取随机数`/`创建列表`/`添加元素`/`取元素`/`删除元素`/`列表长度`/`遍历列表`/`创建字典`/`字典设值`/`字典取值`/`字典删除`/`字典长度`/`读取文件`/`写入文件`/`追加文件`/`删除文件`/`文件是否存在`/`当前时间`/`调试输出`/`退出`/`断言` 等）已**接线**到 `文本_*`/`列表_*`/`字典_*`/`文件_*`/`日期_*` 现代实现，可直接调用；`等待(毫秒)` 为协程保留语义，睡眠请用 `睡眠`。
