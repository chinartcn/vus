...
| **v1.0-beta** | 语言核心稳定 + apk 修复优化 + 插件开发（tui/网络/文件/日期） | 📋 规划中 |
...
| **v5.0** | 正式推广（Gitee、技术社区、示例项目） | 🚀 未来 |
| **v5.1 - v6.x** | 收集用户反馈，整理需求清单 | 🚀 未来 |
| **v7.0** | 包管理器仓库（`vus install` 生态） | 🚀 未来 |
| **v8.0** | 编译器自举（VUS 编译自身） | 🚀 未来 |

```
v0.1 → v0.2 → v1.0-alpha → v1.0-beta → v1.0 → ... → v1.9 → v2.0 → ... → v3.0 → ... → v4.0 → ... → v5.0 → v5.1~v6.x → v7.0 → v8.0
```
