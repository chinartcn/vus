...
class TestParseCommand(unittest.TestCase):
    ...
    def test_options_parsed(self):
        args, opts = self.plugin._parse_command(
            "搜索 hello --索引 书籍 --限制 10 --高亮 标题"
        )
        self.assertEqual(args, ["搜索", "hello"])
        self.assertEqual(opts, {"索引": "书籍", "限制": "10", "高亮": "标题"})

    def test_flag_without_value(self):
        args, opts = self.plugin._parse_command("索引 设置 书籍 --主键")
...
class TestHealthCommand(unittest.TestCase):
    ...
    def test_stats(self):
        plugin = make_plugin()
        client = mock.MagicMock()
        client.get_indexes_stats.return_value = {
            "databaseSize": 1024,
...
class TestIndexCommand(unittest.TestCase):
    ...
    def test_create_index(self):
        ...
        code, out = self.plugin.run(None, "索引 创建 书籍 --主键 id")
        self.assertEqual(code, 0)
        self.client.create_index.assert_called_once_with("书籍", primary_key="id")
    ...
    def test_delete_index(self):
        ...
        code, out = self.plugin.run(None, "索引 删除 书籍")
        self.client.delete_index.assert_called_once_with("书籍")
    ...
    def test_get_settings(self):
        ...
        code, out = self.plugin.run(None, "索引 设置 书籍")
        data = json.loads(out)
...
class TestDocCommand(unittest.TestCase):
    ...
    def test_add_documents_missing_data(self):
        code, out = self.plugin.run(None, "文档 添加 书籍")
    ...
    def test_delete_document(self):
        ...
        code, out = self.plugin.run(None, "文档 删除 书籍 1")
    ...
    def test_get_document(self):
        ...
        code, out = self.plugin.run(None, "文档 获取 书籍 1")
...
class TestSearchCommand(unittest.TestCase):
    ...
    def test_search_basic(self):
        ...
        code, out = self.plugin.run(None, "搜索 苹果 --索引 水果")
        self.assertEqual(code, 0)
    ...
    def test_search_with_options(self):
        ...
        code, out = self.plugin.run(
            None,
            "搜索 苹果 --索引 水果 --筛选 '价格>5' --排序 '价格:desc' "
            "--高亮 名称 --分面 类别 --限制 10",
        )
        self.assertEqual(code, 0)
    ...
    def test_search_invalid_limit(self):
        ...
        code, out = self.plugin.run(None, "搜索 苹果 --索引 水果 --限制 abc")
...
class TestSynonymCommand(unittest.TestCase):
    ...
    def test_get_synonyms(self):
        ...
        code, out = self.plugin.run(None, "同义词 获取 书籍")
        data = json.loads(out)
